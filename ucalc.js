// Copyright 2020 Dylan Sharhon
const $  = id  => document.getElementById(id)
const $$ = css => document.querySelectorAll(css)

$('version').onclick = () => alert('μcalc v1.2.2\n\nBy Dylan Sharhon, 2020\n\nPowered by math.js (Apache 2.0)\n\nUses More Perfect DOS VGA by IBM, Zeh Fernando, and LÆMEUR\n\n🐸')
...
// Cursor navigation
//addHandlers($('up'),     { onPress: display.up })
//addHandlers($('down'),   { onPress: display.down })
addHandlers($('reset'),  { onPress: display.reset })
...
function evaluate () {
  ...
  try {
    ...
    display.print($('input').innerText, out.split('i').join('j'))
    display.bottom()
    display.clear()
  } catch (error) {
    console.error(error)
